...
def load_processed_data(df_train_path, df_predict_path):
    ...
    # --- FILTRAR df_predict_fe_initial por intersección con ListadoIDS.txt ---
    LISTADO_IDS_PATH = '/home/chidiakmartin/gcs-bucket/ListadoIDS.txt'
    try:
        listado_ids_df = pd.read_csv(LISTADO_IDS_PATH, delimiter='\t')
        listado_ids = set(listado_ids_df['product_id'].unique())
    except Exception as e:
        print(f"Error cargando ListadoIDS.txt: {e}")
        listado_ids = set()

    productos_filtrados = set(df_predict_fe_initial['product_id'].unique())
    productos_a_predecir = listado_ids & productos_filtrados
    print(f"Productos a predecir (intersección): {len(productos_a_predecir)}")

    df_predict_fe_initial = df_predict_fe_initial[df_predict_fe_initial['product_id'].isin(productos_a_predecir)].copy()
