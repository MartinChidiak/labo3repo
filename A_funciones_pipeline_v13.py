...
EVENTOS_PATH = os.path.join(GCS_BUCKET_PATH, 'eventos_macro.txt')  

def add_purchase_count(df):
    df = df.sort_values(['customer_id', 'product_id', 'fecha'])
    df['purchase_count'] = (
        df.groupby(['customer_id', 'product_id'])['tn']
        .transform(lambda x: (x > 0).cumsum().shift(1).fillna(0))
    )
    return df

def add_client_product_relationship_age(df):
    first_purchase = df[df['tn'] > 0].groupby(['customer_id', 'product_id'])['fecha'].min().reset_index()
    first_purchase.columns = ['customer_id', 'product_id', 'first_purchase_date']
    df = df.merge(first_purchase, on=['customer_id', 'product_id'], how='left')
    df['relationship_age_months'] = (
        (df['fecha'].dt.year - df['first_purchase_date'].dt.year) * 12 +
        (df['fecha'].dt.month - df['first_purchase_date'].dt.month)
    ).fillna(0)
...
def add_purchase_frequency(df):
    df = df.sort_values(['customer_id', 'product_id', 'fecha'])
    df['period_number'] = df.groupby(['customer_id', 'product_id']).cumcount()
    df['positive_purchases'] = (df['tn'] > 0).astype(int)
    df['cumulative_positive'] = df.groupby(['customer_id', 'product_id'])['positive_purchases'].cumsum()
    df['purchase_frequency'] = df['cumulative_positive'] / (df['period_number'] + 1)
    df.drop(columns=['positive_purchases', 'cumulative_positive', 'period_number'], inplace=True)
    return df


def add_seasonality_features(df, **params):
    df['month_sin'] = np.sin(2 * np.pi * df['fecha'].dt.month / 12)
    df['month_cos'] = np.cos(2 * np.pi * df['fecha'].dt.month / 12)
    df['is_quarter_end'] = df['fecha'].dt.month.isin([3, 6, 9, 12]).astype(int)
    return df


def add_entropy_feature_by_category(df, category_col='cat3', prefix=None, **params):
    """
    Calcula la entropía de compras por cliente según una categoría dada (cat1, cat2, cat3).
    """
    if prefix is None:
        prefix = category_col
    
    cat_counts = df.groupby(['customer_id', category_col]).size().unstack(fill_value=0)
    prob = cat_counts.div(cat_counts.sum(axis=1), axis=0)
    
    # Evitar log(0); np.log(prob + 1e-6) es más numéricamente estable
    entropy = -np.sum(prob * np.log(prob + 1e-6), axis=1)
    
    df = df.merge(entropy.rename(f'{prefix}_entropy'), on='customer_id', how='left')
    return df


def filter_products_by_id(df, product_ids, **params):
    """
    Filtra el DataFrame para mantener solo los productos cuyos IDs están en la lista proporcionada.
    Parameters:
    - df: DataFrame con los datos
    - product_ids: lista o conjunto de IDs de producto a mantener
    """
    print(f"Filtrando productos por IDs: {product_ids}")
    return df[df['product_id'].isin(product_ids)].copy()
...
def fill_missing_product_info(df, productos_df, product_cols_to_fill=None):
    if product_cols_to_fill is None:
        # Asume que quieres rellenar todas las columnas excepto 'product_id'
        product_cols_to_fill = [col for col in productos_df.columns if col != 'product_id']
    df_filled = df.copy()
    df_filled = pd.merge(
        df_filled,
        productos_df[['product_id'] + product_cols_to_fill],
        on='product_id',
        how='left',
